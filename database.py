...
def definir_pin(user_id: str, pin: str):
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET pin_hash=? WHERE id=?", (hasher_pin(pin), user_id))
    conn.commit()
    conn.close()
...
def obtenir_id_par_prenom(nom_reel: str) -> str | None:
    conn   = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT id FROM users WHERE real_name=?", (nom_reel.strip().capitalize(),))
...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS profils (
            ...
            prenom          TEXT, situation TEXT, defis TEXT,
            objectifs       TEXT, humeur_generale TEXT,
            preferences     TEXT, notes_aura TEXT, derniere_maj TEXT,
            FOREIGN KEY (user_id) REFERENCES users(id)
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS humeurs (
            ...
            user_id TEXT, score INTEGER, emoji TEXT, note TEXT, date TEXT,
            FOREIGN KEY (user_id) REFERENCES users(id)
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS compteur_quotidien (
            user_id TEXT, date TEXT, nb INTEGER DEFAULT 0,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS feedbacks (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id TEXT, utile TEXT, commentaire TEXT,
            prix_mois TEXT, timestamp TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(id)
        )
    """)
    cursor.execute("""
...
def nettoyer_messages_corrompus(user_id: str) -> int:
    ...
    cursor.execute("""
        ...
        AND (content IN ('user','assistant') OR content IS NULL
             OR content='' OR length(content)<2)
    """, (user_id,))
    nb = cursor.rowcount
    conn.commit()
    conn.close()
...
def sauvegarder_conversation(user_id: str, role: str, texte: str):
    if role not in ("user","assistant") or not texte or not texte.strip():
        return
    try:
        conn   = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
    ...
    try:
        ...
        cursor.execute(
            """SELECT role,content,timestamp FROM messages
               ...
               AND content NOT IN ('user','assistant') AND length(content)>=2
        ...
        for r in rows:
            ...
            if (r[0] in ("user","assistant") and contenu
...
def compter_messages_aujourdhui(user_id: str) -> int:
    conn = sqlite3.connect(DB_PATH)
    c    = conn.cursor()
    c.execute("SELECT nb FROM compteur_quotidien WHERE user_id=? AND date=?",
...
def incrementer_compteur_quotidien(user_id: str):
    conn = sqlite3.connect(DB_PATH)
    c    = conn.cursor()
    c.execute("""INSERT INTO compteur_quotidien (user_id,date,nb) VALUES (?,?,1)
...
def supprimer_compte_complet(user_id: str):
    conn = sqlite3.connect(DB_PATH)
    c    = conn.cursor()
    for t in ["messages","profils","humeurs","compteur_quotidien","feedbacks","logs_acces"]:
...
def sauvegarder_profil(user_id: str, donnees: dict):
    try:
        ...
        """, (
            user_id, donnees.get("prenom",""), donnees.get("situation",""),
...
def charger_humeurs(user_id: str, jours: int = 14) -> list:
    ...
    c.execute("SELECT score,emoji,note,date FROM humeurs WHERE user_id=? AND date>=? ORDER BY date ASC",
              (user_id, depuis))
    rows = c.fetchall()
    conn.close()
    return [{"score":r[0],"emoji":r[1],
...
def sauvegarder_feedback(user_id: str, utile: str, commentaire: str, prix_mois: str):
    conn = sqlite3.connect(DB_PATH)
    c    = conn.cursor()
    c.execute("INSERT INTO feedbacks (user_id,utile,commentaire,prix_mois,timestamp) VALUES (?,?,?,?,?)",
...
def a_deja_donne_feedback_aujourd_hui(user_id: str) -> bool:
    conn = sqlite3.connect(DB_PATH)
    c    = conn.cursor()
    c.execute("SELECT COUNT(*) FROM feedbacks WHERE user_id=? AND timestamp>=?",
              (user_id, datetime.now().strftime("%Y-%m-%d")))
    nb = c.fetchone()[0]
    conn.close()
    return nb > 0
